...
struct fx_framebuffer {
	...
	bool external_only;
	uint32_t drm_format; // only set for custom-allocated offscreen buffers
...
void fx_framebuffer_get_or_create_custom(struct fx_renderer *fx_renderer,
		...
		uint32_t preferred_drm_format, struct fx_framebuffer **fx_buffer,
		bool *failed);
...
struct fx_renderer {
	...
	// 3D LUT baked from the current output color transform (HDR)
	GLuint lut3d_tex;
	struct wlr_color_transform *lut3d_src;
