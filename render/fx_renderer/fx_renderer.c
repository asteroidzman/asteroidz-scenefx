...
static void fx_renderer_destroy(struct wlr_renderer *wlr_renderer) {
	struct fx_renderer *lut_renderer = fx_get_renderer(wlr_renderer);
	if (lut_renderer->lut3d_tex != 0) {
		glDeleteTextures(1, &lut_renderer->lut3d_tex);
	}
	if (lut_renderer->lut3d_src != NULL) {
		wlr_color_transform_unref(lut_renderer->lut3d_src);
	}
	struct fx_renderer *renderer = fx_get_renderer(wlr_renderer);
...
static struct wlr_render_pass *begin_buffer_pass(struct wlr_renderer *wlr_renderer,
	...
	if (pass != NULL && options->color_transform != NULL &&
...
static bool link_shaders(struct fx_renderer *renderer) {
	...
	// output color transform (HDR) resolve shader
	if (renderer->exts.OES_texture_3D &&
			!link_color_transform_program(&renderer->shaders.color_transform)) {
		wlr_log(WLR_ERROR, "Could not link color transform shader");
		goto error;
	}
...
struct wlr_renderer *fx_renderer_create_egl(struct wlr_egl *egl) {
	...
	renderer->exts.OES_texture_float_linear =
		check_gl_ext(exts_str, "GL_OES_texture_float_linear");
	if (check_gl_ext(exts_str, "GL_OES_texture_3D")) {
		renderer->exts.OES_texture_3D = true;
		load_gl_proc(&renderer->procs.glTexImage3DOES, "glTexImage3DOES");
	}

	// Output color transforms (HDR) are applied via a baked 3D LUT
	renderer->wlr_renderer.features.output_color_transform =
		renderer->exts.OES_texture_3D && renderer->exts.OES_texture_float_linear;
