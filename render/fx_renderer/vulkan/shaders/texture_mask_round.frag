...
// scenefx masked rounded-texture shader (fx_vk fork). Identical colour + corner
// path to texture_round.frag, plus a transparency-mask cutout used by the
// per-window/layer blur "ignore_transparent" path: the blur cache is sampled at
// set 0 (like texture_round) and a mask surface is sampled at set 1. Where the
// mask's alpha is below a threshold the blur is made fully transparent, so the
// unblurred background shows through the mask's transparent holes. Mirrors the
// GLES fx_render_pass_add_blur stencil-mask path (ignore_transparent +
// discard_alpha_threshold == blur_data.transparency_threshold).

layout(set = 0, binding = 0) uniform sampler2D tex;       // blur cache image
layout(set = 1, binding = 0) uniform sampler2D mask_tex;  // transparency mask

layout(location = 0) in vec2 uv;
// Box-relative unit coordinate from the vertex shader (see common.vert). Spans
// the full destination box; used both for corner geometry and to sample the
// mask (which covers the same dst box as the blur).
layout(location = 1) in vec2 box_pos;
layout(location = 0) out vec4 out_color;

// Push-constant layout (fragment stage), matching pass.c and the frag range of
// tex_mask_pipe_layout (80..244):
//   struct fx_vk_frag_texture_pcr_data @ 80  (matrix, alpha, luminance)
//   struct fx_vk_frag_corner_pcr_data  @ 160 (64 bytes, ends at 224)
//   struct fx_vk_frag_mask_pcr_data    @ 224 (mask_uv_off, mask_uv_size, threshold)
layout(push_constant, row_major) uniform UBO {
	...
	layout(offset = 224) vec2 mask_uv_off;
	vec2 mask_uv_size;
	float threshold;
} data;
...
void main() {
	...
	// Transparency mask: the mask covers the same dst box, sampled via box_pos.
	// Blur vanishes where the mask's alpha is below the threshold, so the
	// unblurred background shows through the mask's transparent holes.
	vec2 mask_uv = data.mask_uv_off + box_pos * data.mask_uv_size;
	float m = textureLod(mask_tex, mask_uv, 0).a;
	float mask_cov = step(data.threshold, m);
	out_color *= mask_cov;
}
