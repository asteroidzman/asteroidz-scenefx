...
bool link_box_shadow_program(struct box_shadow_shader *shader) {
	...
	shader->size = glGetUniformLocation(prog, "size");
	shader->blur_sigma = glGetUniformLocation(prog, "blur_sigma");
	shader->corner_radii.top_left = glGetUniformLocation(prog, "corner_radius_top_left");
	shader->corner_radii.top_right = glGetUniformLocation(prog, "corner_radius_top_right");
	shader->corner_radii.bottom_left = glGetUniformLocation(prog, "corner_radius_bottom_left");
	shader->corner_radii.bottom_right = glGetUniformLocation(prog, "corner_radius_bottom_right");
	shader->clip_position = glGetUniformLocation(prog, "clip_position");
	shader->clip_size = glGetUniformLocation(prog, "clip_size");
	shader->clip_radius.top_left = glGetUniformLocation(prog, "clip_radius_top_left");
	shader->clip_radius.top_right = glGetUniformLocation(prog, "clip_radius_top_right");
...
bool link_color_transform_program(struct color_transform_shader *shader) {
	GLuint prog;
	shader->program = prog = link_program(color_transform_frag_src);
	if (!shader->program) {
		return false;
	}
	shader->proj = glGetUniformLocation(prog, "proj");
	...
	shader->lut_scale = glGetUniformLocation(prog, "lut_scale");
	shader->lut_offset = glGetUniformLocation(prog, "lut_offset");
	return true;
}
