...
void fx_framebuffer_get_or_create_custom(struct fx_renderer *renderer,
		...
		uint32_t preferred_drm_format, struct fx_framebuffer **fx_framebuffer,
		bool *failed) {
	if (*failed) {
		return;
	}

	// Get the best supported DRM format (DMABUF if supported). A deep
	// (e.g. 10-bit / half-float) format may be requested so that effects
	// don't lose precision on deep (10-bit / HDR) outputs; fall back to
	// 8-bit when it isn't supported.
	...
	const struct wlr_drm_format *format = NULL;
	if (preferred_drm_format != 0) {
		format = wlr_drm_format_set_get(texture_formats, preferred_drm_format);
	}
	if (format == NULL) {
		const struct fx_pixel_format *pix_format =
			get_fx_format_from_gl(GL_RGBA, GL_UNSIGNED_BYTE, has_alpha);
		format = wlr_drm_format_set_get(texture_formats, pix_format->drm_format);
	}
	if (format == NULL) {
	...
	if (*fx_framebuffer != NULL) {
		...
		if (wlr_buffer != NULL) {
			if (wlr_buffer->width == width && wlr_buffer->height == height &&
					(*fx_framebuffer)->drm_format == format->format) {
				return;
			}
	...
	if (wlr_buffer == NULL && format->format != DRM_FORMAT_ARGB8888 &&
			format->format != DRM_FORMAT_XRGB8888) {
		// The renderer supports the deep format but the allocator refused
		// it: retry with the plain 8-bit format instead of disabling
		// effects entirely.
		const struct fx_pixel_format *pix_format =
			get_fx_format_from_gl(GL_RGBA, GL_UNSIGNED_BYTE, has_alpha);
		format = wlr_drm_format_set_get(texture_formats, pix_format->drm_format);
